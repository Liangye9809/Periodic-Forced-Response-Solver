...
for j = 1:size(indx, 2)
    ...
    plot(t, xct_poss(:, 3), 'bo-', 'LineWidth', 2, 'DisplayName', '$x_n$'), grid on;
    legend show
    titlename = 'Omega = ' + string(omega_poss) + ', $max(|x_n|)=$ ' + string(max(abs(xct_poss(:, 3))));
    title(titlename);
    subplot(2,size(indx,2),j+size(indx,2)) % Force
    plot(t, Ft_poss(end - N + 1:end, 3), 'bo-', 'LineWidth', 2, 'DisplayName', '$F_n$'), grid on;
    legend show
    
    figure(101)
    subplot(2,size(indx,2),j) % displacement xt
    plot(t, xct_poss(:, 1), 'bo-', 'LineWidth', 2, 'DisplayName', '$x_t$'), grid on;
    legend show
    titlename = 'Omega = ' + string(omega_poss) + ', $max(|x_t|)=$ ' + string(max(abs(xct_poss(:, 1))));
    title(titlename);
    subplot(2,size(indx,2),j+size(indx,2)) % Force
    plot(t, Ft_poss(end - N + 1:end, 1), 'bo-', 'LineWidth', 2, 'DisplayName', '$F_t$'), grid on;
    legend show
    ...
    plot(t, Ft_poss(end - N + 1:end, 1), 'bo-', 'LineWidth', 2);
    legend('mu * Fn', '-mu * Fn', 'T1');legend show

    figure(103)
    subplot(1,size(indx,2),j) % flag
    FlagTemp(:, 1) = flag_poss(1,1,end - N + 1:end);
    plot(t, FlagTemp, 'b.'), grid on;
    slip_m = sum(FlagTemp(find(FlagTemp == -1)));
    slip_p = sum(FlagTemp(find(FlagTemp == 1)));
    titlename = 'Omega = ' + string(omega_poss) + ', +' + string(slip_p) + ', -' + string(slip_m);
    title(titlename);
...
%%
figure(501)
ind_gap_stick = find(P.gap_cont == 1 & (P.slipP_cont + P.slipM_cont) == 0);
ind_gap = find(P.gap_cont == 1);
