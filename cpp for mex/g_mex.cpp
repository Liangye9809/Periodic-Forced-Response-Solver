...
const mxArray* getFieldOrProperty(const mxArray *obj, const char *name) {
    ...
    mexErrMsgIdAndTxt("g_mex:inputType", "p.fc must be a struct or object.");
    return nullptr;
}
...
{
    if (nrhs != 2) {
        mexErrMsgIdAndTxt("g_mex:nrhs", "Two inputs required: xt, pc.");
    }
    if (nlhs != 1) {
        mexErrMsgIdAndTxt("g_mex:nlhs", "One output required: F struct.");
    }
    ...
    for (int j = 0; j < 2; j++) {
        for (mwSize i = 0; i < N; i++) {
            ...
            // 2. call gf_core
            gf_core(xrow.data(), Np, kn, xn0, mu, kt, w_data, Frow.data());
            // 3. Store row into F.F
            for (mwSize k = 0; k < Nx; k++) {
                F_data[i + k*N] = Frow[k]; // column-major
            }
        }
    }
