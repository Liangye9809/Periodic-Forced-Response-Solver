function JNL = HBMJACOB_analytical_W_gf_2dofs_2(dx, kn, mu, kt, H, N, Mft)
    ...
    if C_ps == 1
        [MW, ~] = fW(0, 2*pi, H);
        JNL(1:2 * H + 1, 1:2 * H + 1) = JNL(1:2 * H + 1, 1:2 * H + 1) + kt .* MW;
    ...
    while C_gs(i, 1) >= 0
        ...
        JNL(1:2 * H + 1, 2 * H + 1 + 1:end) = JNL(1:2 * H + 1, 2 * H + 1 + 1:end) + Mw * (kt .* dxt ./ dxn .* c_vec);
        i = i + 1;
    end
