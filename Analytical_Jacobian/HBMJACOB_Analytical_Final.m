function [JNL, Mft, JNLt, F, wt] = HBMJACOB_Analytical_Final(xt, kn, xn0, mu, kt, w_in, H, N, nloop)
    ...
    if ismember(0, a) % slip or separation happene
        ...
        cg_xn = c .* a .* exp(cos(t_s + 1)) .* sin(t_s + 1) ./ (2 .* cos(sin(t_s)) .* cos(t_s)); % stick part in gap to stick
        JNLt(1:N, 1) = JNLt(1:N, 1) - a .* 0.5 .* kt;

        % JNLt(1:N, 2 * H + 2) = JNLt(1:N, 2 * H + 2) + c_xn .* 0.5 .* kn .* mu; % slip to stick
        JNLt(1:N, 2 * H + 2) = JNLt(1:N, 2 * H + 2) + kt .* cg_xn .* 0.5; % gap to stick
