function [F, w, flag] = gf(x, kn, xn0, mu, kt, w_in, xn_pre,...
    dxt, dxt_m, flag_m, N) % x is a 3*Nx dof row vector, represent 2*Nx tangential directions and 1*Nx normal direction
    ...
    for i = 1:Nx
        ...
        [F(indx1), w(1, i), flag(1, i), ~] = TangentialForces(x(indx1), w(1, i), kt(1, i), mu(1, i), F(indx3), x(indx3), xn_pre(i),...
            dxt_m(indx1), dxt(indx1), dxt_m(indx3), dxt(indx3), flag_m(1, i, 1), N, kn(i));
        [F(indx2), w(2, i), flag(2, i), ~] = TangentialForces(x(indx2), w(2, i), kt(2, i), mu(2, i), F(indx3), x(indx3), xn_pre(i),...
            dxt_m(indx2), dxt(indx2), dxt_m(indx3), dxt(indx3), flag_m(2, i, 1), N, kn(i));
    end
