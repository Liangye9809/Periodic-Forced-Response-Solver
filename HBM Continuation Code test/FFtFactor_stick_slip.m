function [dft, M_fstar] = FFtFactor_stick_slip(ft_in, xt, flag, kt, kn, mu, EH)
    ...
    for i = 1:Nx
        ...
        xt1 = xt(:, 3 * i - 2);
        xt2 = xt(:, 3 * i - 1);
        [dft(:, 3 * i - 2), M_fstar(:, 3 * i - 2)] = FFtFactor_perT(ft_in(:, 3 * i - 2), xt1, xn, flagT1, kt(1, i), kn(i), mu(1, i), EH);
        [dft(:, 3 * i - 1), M_fstar(:, 3 * i - 1)] = FFtFactor_perT(ft_in(:, 3 * i - 1), xt2, xn, flagT2, kt(2, i), kn(i), mu(2, i), EH);
    end
end

function [v_df, v_fstar] = FFtFactor_perT(Ft_in, xt, xn, flag, kt, kn, mu, EH) % all the dofs of xt, and Ft
    v_fstar = 0;
    v_df = 0;
    N = size(xt, 1);
...
function [v_df, f_star, dt_star] = scaleSlipStick(f1_m, f1_p, f2_m, f2_p, N, EH, i_m, i_p)
    fdt = (f2_m - f1_m) / (f1_p - f1_m + f2_m - f2_p);
    f_star = f1_m + (f1_p - f1_m) * fdt;
    dt_star = fdt * 2 * pi / N;
